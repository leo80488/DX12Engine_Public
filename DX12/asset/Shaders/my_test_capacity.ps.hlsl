...
Texture2D SurfaceMap   : register(t2, space3);
Texture2D EmissiveMap  : register(t3, space3);
Texture2D DetailMap    : register(t4, space3);
...
GOut main(PSIn i)
{
    ...
    float roughness = (RoughnessOverride > 0.0)
        ? RoughnessOverride
        : SurfaceMap.Sample(g_LinearWrap, uv).g;
    float metalness = (MetalnessOverride > 0.0)
        ? MetalnessOverride
        : SurfaceMap.Sample(g_LinearWrap, uv).b;
    float ao  = AmbientOcclusionOverride > 0.0 
        ? AmbientOcclusionOverride 
        : SurfaceMap.Sample(g_LinearWrap, uv).r;
    float reflect_  = 0.5;
