...
void SSRDepthHierarchyPass::EnsureTexture(uint32_t depthW, uint32_t depthH)
{
    ...
    // Match screen resolution directly (not pow2) so the shader's UV→mip-texel
    // mapping matches Wicked's convention: `mipCoord = uv * screenRes * 0.5^mip`.
    // Odd-dim mip reductions handled by the reduce shader clamping edge loads.
