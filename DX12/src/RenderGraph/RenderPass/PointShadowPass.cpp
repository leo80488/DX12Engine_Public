...
static constexpr uint32_t kInstanceBufSlot   = 8;   // root SRV t0 space0 (unused; indirect draw handles it)
...
// ---------------------------------------------------------------------------
void PointShadowPass::Init(IGraphicsDevice& gfx)
{
    m_gfxPtr = &gfx;

    // Reuse the CSM/Spot shadow shaders — depth-only with a single
    // ShadowPerViewCB at b1 space0, exactly what each cube face needs.
    ...
    if (!m_psoCache.GetOrCreate(BuildPSODesc(basePerm)))
        LOG_ERROR("PointShadowPass: base PSO creation failed");

    // ---- Cube shadow atlas: D32_FLOAT array of 6*kMaxCasters slices, TEXTURECUBE
    // misc flag → the RHI builds a TextureCubeArray<float> SRV + per-face DSVs.
    {
        RHI::TextureDesc td;
        ...
        td.mip_levels = 1;
        td.array_size = kFaceCount;                       // 6 * kMaxCasters
        td.misc_flags = RHI::ResourceMiscFlag::TEXTURECUBE;
        td.clear      = RHI::ClearValue::DepthStencil(0.0f, 0); // reversed Z
        td.debug_name = "PointShadowPass.CubeAtlas";
        if (!gfx.CreateTexture(td, m_atlas))
            LOG_ERROR("PointShadowPass: cube shadow atlas creation failed");
    }

    // ---- Per-face CB pool (triple-buffered, 256B-aligned per face) ---------
    if (!m_faceCBs.Create(gfx, "PointShadowPass.FaceCBs"))
        LOG_ERROR("PointShadowPass: face CB pool creation failed");

    // ---- Indirect draw command buffer (same layout as SpotShadowPass) ------
    {
    ...
    // Linear-wrap sampler for alpha-test PS (matches GBuffer.ps's g_LinearWrap).
    {
        ...
        if (!gfx.CreateSampler(sd, m_alphaSamplerIdx))
            LOG_ERROR("PointShadowPass: alpha-test sampler creation failed");
    }

    LOG_SUCCESS("PointShadowPass: initialised (Cube %ux%u x %u cubes, indirect=%s)",
                kShadowMapSize, kShadowMapSize, kMaxCasters,
                m_indirectArgMapped[0] ? "YES" : "NO");
}

// ---------------------------------------------------------------------------
PSODesc PointShadowPass::BuildPSODesc(PermutationKey perm) const
{
    PSODesc desc;
    ...
    // CullMode::None: thin/open occluders must still cast on all 6 faces.
    desc.rs.cull_mode = RHI::CullMode::NONE;
    desc.rs.depth_clip_enable = false;

    // Slope-scale + constant bias (reversed-Z, D32_FLOAT, GREATER_EQUAL). Same
    // convention as SpotShadowPass — all bias on the caster side so the
    // receiver-side compare in Lighting.ps stays unbiased (avoids leaks).
...
// ---------------------------------------------------------------------------
uint64_t PointShadowPass::GetCubeAtlasSrvHandle() const
{
...
// ---------------------------------------------------------------------------
void PointShadowPass::UploadFaceCBs()
{
    if (!m_gfxPtr) return;
    auto* pool = m_faceCBs.Current(*m_gfxPtr);
    if (!pool) return;
    const uint32_t activeFaces = m_activeCount * kFacesPerCube;
    for (uint32_t i = 0; i < activeFaces; ++i)
    {
        XMMATRIX m = XMLoadFloat4x4(&m_pendingVP[i]);
        XMFLOAT4X4 t;
        XMStoreFloat4x4(&t, XMMatrixTranspose(m));
        std::memcpy(pool->slots + i * kFaceCBStride, &t, sizeof(XMFLOAT4X4));
    }
}

// ---------------------------------------------------------------------------
RHI::CommandList PointShadowPass::Execute(RHI::CommandList cl)
{
    if (!m_atlas.IsValid()) return cl;

    // No shadow-casting point lights → skip the clear/render entirely. The very
    // first frame still needs a one-shot DEPTHSTENCIL → SRV transition so
    // LightingPass can bind the cleared cube (reversed-Z 0 == "no shadow").
    if (m_activeCount == 0)
    {
        if (m_firstExecution)
        {
    ...
    // On subsequent frames the atlas arrives in DEPTH_READ_SRV (sampled last
    // frame). Transition back to DEPTHSTENCIL for writes.
    if (!m_firstExecution)
    {
        cl.PushBarrier(RHI::GPUBarrier::Image(
            &m_atlas,
    ...
    // Always clear every active face — stale depth would corrupt the compare.
    for (uint32_t s = 0; s < m_activeCount * kFacesPerCube; ++s)
        gfx.ClearDepthStencilSlice(m_atlas, s, 0.0f, 0, cl); // reversed Z

    // Pull the draw lists we need ONCE (shared across all faces).
    ...
    if (opaqueDraws.empty() && shadowDraws.empty() && transparentDraws.empty())
    {
        cl.PushBarrier(RHI::GPUBarrier::Image(
            &m_atlas,
            RHI::ResourceState::DEPTHSTENCIL,
    ...
    // Build indirect arg buffer once, split alpha / non-alpha — same layout as
    // SpotShadowPass. Reused unchanged for every cube face.
    ...
    // ---- Render each active cube's 6 faces into its slices -----------------
    const RHI::GPUBuffer& faceCBBuf = m_faceCBs.CurrentBuffer(gfx);
    if (!faceCBBuf.IsValid()) return cl;

    for (uint32_t face = 0; face < m_activeCount * kFacesPerCube; ++face)
    {
        gfx.SetDepthStencilSlice(m_atlas, face, cl);
