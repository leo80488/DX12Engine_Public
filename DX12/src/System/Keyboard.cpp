...
{
    // Pressed this frame, but not last frame = triggered
    return keystates[keycode] && !keystatesLastFrame[keycode];
}
...
{
    keystates.reset();
    keystatesLastFrame.reset();
}

void Keyboard::EndOfFrame()
{
    // Update last frame's key state
