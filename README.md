# DX12Engine — Feature Overview

A real-time rendering engine and editor written in C++20 / Direct3D 12 (Shader Model 6.6, DXIL via DXC).
The project is a personal R&D codebase that combines a deferred clustered renderer, a render-graph,
an ECS, a Lua scripting layer, an offline asset cooker, and a full ImGui editor with a separate
shader-authoring sandbox (`ShaderLab.exe`).

> Built against Visual Studio 2022, Windows 10 SDK, MSVC v143, C++20.
> Solution `DX12Engine.sln` produces 4 binaries:
> `EngineCore.lib`, `Editor.exe`, `Game.exe`, `ShaderLab.exe`.
